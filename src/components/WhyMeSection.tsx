...
const WhyMeSection = () => {
  ...
  return (
    <section className="py-12 md:py-20 relative">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] md:w-[800px] h-[300px] md:h-[400px] bg-primary/5 rounded-full blur-3xl" />
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 max-w-5xl mx-auto">
          {reasons.map((reason, index) => (
            <div
              key={reason.title}
              className="glass-card p-4 md:p-6 hover:border-primary/30 transition-all duration-300 group animate-fade-in hover:-translate-y-2"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className={`w-10 md:w-12 h-10 md:h-12 rounded-xl bg-muted flex items-center justify-center mb-3 md:mb-4 group-hover:scale-110 transition-transform duration-300 ${reason.color}`}>
                <reason.icon className="w-5 md:w-6 h-5 md:h-6" />
              </div>
              <h3 className="text-base md:text-lg font-semibold text-foreground mb-2">{reason.title}</h3>
